"""
MBB beam optimization using sensitivity filter
...
function simp1(volfrac, rρ)
    ...
    try
        # plot initial design
        display(image(reshape(-x.variables, nely, nelx)', interpolate=false, axis=(aspect=DataAspect(), yreversed=true, xreversed=true)))
        ...
        for loop in 1:maxiter+1
            ...
            empty!(current_axis())
            image!(reshape(-x.variables, nely, nelx)', interpolate=false)
