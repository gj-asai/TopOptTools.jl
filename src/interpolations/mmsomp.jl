mutable struct MMSOMP{M,N} <: MaterialInterpolation{N}
    mat::Vector{Material}
    penal::Real
end
MMSOMP(mat::Vector{<:Material}, penal::Real) = MMSOMP{length(mat),length(mat) + 1}(mat, penal)

function interpolate(xe::Vector{T}, interp::MMSOMP) where {T<:Real}
...
function compliance(_, results::FEResults, ::FEModel{dim,nvar,interp}) where {dim,nvar,interp<:MMSOMP}
    @unpack K, u = results
    return u' * K * u
end

function dcompliance(_, results::FEResults, model::FEModel{dim,nvar,interp}) where {dim,nvar,interp<:MMSOMP}
...
function impact(x, ::FEResults, model::FEModel{dim,nvar,interp}) where {dim,nvar,interp<:MMSOMP}
    CO2 = 0
...
function dimpact(x, ::FEResults, model::FEModel{dim,nvar,interp}) where {dim,nvar,interp<:MMSOMP}
    dCO2dx = zero(x)
    for (i, mati) in enumerate(model.mat_interp.mat)
...
function volume(x, ::FEResults, model::FEModel{dim,nvar,interp}) where {dim,nvar,interp<:MMSOMP}
    ρsum = [sum(x[nvar*(i-1)+1:nvar*i-1]) for i in 1:getncells(model.grid)]
    return ρsum ⋅ model.elemvol / sum(model.elemvol)
end

function dvolume(x, ::FEResults, model::FEModel{dim,nvar,interp}) where {dim,nvar,interp<:MMSOMP}
    ∂g∂x = zero(x)
